...
class KaryawanDataTable extends DataTable
{
    ...
    {
        return datatables()
            ->eloquent($query->with('limit'))
            ->skipTotalRecords()
            ->addColumn('foto', function ($row) {
                if ($row->foto) {
                    return '<img class="rounded" style="width:90px; height:90px;" src="' . asset('storage/images/karyawan/small_' . $row->foto) . '" />';
                }
            })
            ->addColumn('action', function ($row) {
                ...
                if ($user->can('update ' . request()->path())) {
                    $action .= '<a class="dropdown-item action" data-method="GET" href="' . route('master-data.karyawan.edit', $row->id) . '">Edit</a>';
                }
    ...
    protected function getColumns()
    {
        return [
            ...
            Column::make('foto'),
            Column::make('divisi'),
            Column::make('limit.nominal')->title('limit'),
            Column::make('aktif'),
