<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title" id="myModalLabel16">{{ $data->id ? 'Edit Karyawan' : 'Tambah Karyawan' }}</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
        <form id="form-action" method="POST"
            action="{{ $data->id ? route('master-data.karyawan.update', $data->id) : route('master-data.karyawan.store') }}">
            @csrf
            @if ($data->id)
                @method('put')
            @endif
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <label for="">Limit</label>
                         <input type="text" value="{{ $data->limit->nominal_formatted ?? 0}}" onkeyup="this.value=formatAngka(this.value)" name="limit" class="form-control">
                    </div>
                </div>

                <div class="col-md-12 avatar-group mt-1">
                    <img src="{{ $data->foto ? asset('storage/images/karyawan/medium_' . $data->foto) : asset('assets/images/avatars/delivery.jpg') }}"
                        alt="delivery avatar"
