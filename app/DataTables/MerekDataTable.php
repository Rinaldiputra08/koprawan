    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('foto', function($row){
                if($row->foto){
                    return '<img class="rounded" style="width:90px; height:90px;" src="'.asset('storage/images/produk/small_'.$row->foto->nama_file).'" />';
                }
            })
            ->addColumn('action', function($row){
                ...
                if ($user->can('update ' . request()->path())) {
                    $action .= '<a class="dropdown-item action" data-method="GET" href="'.route('master-data.merek.edit',$row->id).'">Edit</a>';
                }
