                ...
                <div class="card">
                    ...
                    <div class="col-md-4">
                        @can('create '.request()->segment(1).'/'.request()->segment(2))
                            <button type="button" data-url="{{ route('master-data.supplier.create') }}" class="btn btn-gradient-primary add">Tambah</button>
                        @endcan
                    </div>
                    <div class="card-body">
                        {!! $dataTable->table() !!}
...
@push('js')
    ...
    <script>
        ...
        var supplierJs = function() {
            const formAction = '#form-action', datatable = 'supplier-table'
