        ...
        return datatables()
            ...
            ->addColumn('foto', function($row){
                if($row->fotoThumbnail){
                    return '<img class="rounded" style="width:90px; height:90px;" src="'.asset('storage/images/produk/small_'.$row->fotoThumbnail->nama_file).'" />';
                }
            })
            ->addColumn('action', function($row){
                ...
                $action .= '<a class="dropdown-item action" data-method="GET" href="'.route('master-data.produk.show',$row->id).'">Detail</a>';
                
                if ($user->can('update ' . request()->path())) {
                    $action .= '<a class="dropdown-item action" data-method="GET" href="'.route('master-data.produk.edit',$row->id).'">Edit</a>';
                }
