                ...
                <div class="card">
                    ...
                    <div class="col-md-4">
                        @can('create '.request()->segment(1).'/'.request()->segment(2))
                            <button type="button" data-url="{{ route('master-data.kategori.create') }}" class="btn btn-gradient-primary add">Tambah</button>
                        @endcan
                    </div>
                    <div class="card-body">
                        {!! $dataTable->table() !!}
...
@push('js')
    ...
    <script>
        ...
        var kategoriJs = function() {
            const formAction = '#form-action', 
            datatable = 'kategori-table'
            ...
            //             allowed_extension = ['jpg', 'jpeg', 'JPG', 'JPEG','png']
            ...
            function previewImage()
            {
                $('#change-picture').on('change', function(event){
                    ...
                    const imageFrom = this.files[0]
                    const output = document.querySelector('.category-img')
                    output.src = URL.createObjectURL(imageFrom);
